//! UNSW-NB15 (CICFlowMeter-processed) dataset loader.
//!
//! Loads `Data.csv` (76 numeric feature columns) and `Label.csv` (integer labels 0-9),
//! maps labels to the standard 5-class scheme, and produces a [`DatasetSplit`].
...
/// Number of feature columns in the CICFlowMeter-processed UNSW-NB15 Data.csv.
const NUM_FEATURES: usize = 76;

/// 76 feature column names from Data.csv header.
...
/// Map UNSW-NB15 integer labels (0-9) to the 5-class scheme.
///
/// From Readme.txt:
...
fn unsw_attack_category(label: usize) -> usize {
    match label {
        0 => 0,          // Benign -> Normal
        3 | 6 => 1,      // DoS, Generic -> DoS
        1 | 5 | 7 => 2,  // Analysis, Fuzzers, Reconnaissance -> Probe
        4 | 9 => 3,      // Exploits, Worms -> R2L
        2 | 8 => 4,      // Backdoor, Shellcode -> U2R
        other => {
...
/// Load the UNSW-NB15 CICFlowMeter-processed dataset.
///
/// * `data_path` — path to `Data.csv` (76 numeric columns)
/// * `label_path` — path to `Label.csv` (single column: integer 0-9)
///
/// Splits 70% train / 12% val / 18% test.
...
mod tests {
    ...
    #[test]
    fn test_unsw_label_mapping() {
        assert_eq!(unsw_attack_category(0), 0); // Benign -> Normal
        assert_eq!(unsw_attack_category(3), 1); // DoS -> DoS
        assert_eq!(unsw_attack_category(6), 1); // Generic -> DoS
        assert_eq!(unsw_attack_category(1), 2); // Analysis -> Probe
        assert_eq!(unsw_attack_category(5), 2); // Fuzzers -> Probe
        assert_eq!(unsw_attack_category(7), 2); // Reconnaissance -> Probe
        assert_eq!(unsw_attack_category(4), 3); // Exploits -> R2L
        assert_eq!(unsw_attack_category(9), 3); // Worms -> R2L
        assert_eq!(unsw_attack_category(2), 4); // Backdoor -> U2R
        assert_eq!(unsw_attack_category(8), 4); // Shellcode -> U2R
    }
}
